...
class ClassList:
    ...
    def __init__(self, max_students):
        # check data is valid -- gatekeeper pattern
        if max_students <= 0:
            raise StudentError('Cant add a negative student')
    ...
    def add_student(self, student):
        ...
        if len(self.class_list) < self.max_students:
            if student not in self.class_list:
                ...
                raise StudentError(
                    'Student %s already enrolled, can\'t add again' % student)
    ...
    def is_class_full(self):
        """ Returns true if class is full otherwise false"""
        if len(self.class_list) == self.max_students:
            return True
        return False
